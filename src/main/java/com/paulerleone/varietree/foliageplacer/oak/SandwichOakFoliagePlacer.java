...
public class SandwichOakFoliagePlacer extends FoliagePlacer {
    public static final Codec<SandwichOakFoliagePlacer> CODEC = RecordCodecBuilder.create(SandwichOakFoliagePlacerInstance
            -> foliagePlacerParts(SandwichOakFoliagePlacerInstance).and(Codec.intRange(0, 16).fieldOf("height")
            .forGetter(fp -> fp.height)).apply(SandwichOakFoliagePlacerInstance, SandwichOakFoliagePlacer::new));
    private final int height;

    public SandwichOakFoliagePlacer(IntProvider pRadius, IntProvider pOffset, int height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacers.SANDWICH_OAK_FOLIAGE_PLACER.get();
    }
